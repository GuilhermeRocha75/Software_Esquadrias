...
def compare_order_total(unit_cost: float, quantity: int, expected_excel_total: float):
    """Compara sempre o TOTAL do pedido com o total informado pelo Excel."""
    engine_total = float(unit_cost) * int(quantity)
    difference = engine_total - float(expected_excel_total)
    difference_pct = (difference / expected_excel_total * 100.0) if expected_excel_total else 0.0
    return engine_total, difference, difference_pct

class EsquadriasTester(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Software Esquadrias — Tester CR v0.2.1.1")
    ...
    def _build_ui(self):
        ...
        ttk.Label(header, text="Software Esquadrias — Tester CR v0.2.1", style="Title.TLabel").grid(
    ...
    def _build_input_panel(self, parent):
        ...
        ttk.Label(frame, text="TOTAL esperado no Excel (R$)").grid(row=row, column=0, sticky="w", pady=4)
        ...
        ttk.Button(tests, text="Exportar BOM CSV (comparar com Excel)", command=self.export_bom_csv).grid(
    ...
    def _build_result_panel(self, parent):
        ...
        self._card(summary, 1, "Custo técnico / unidade", self.unit_cost_value)
        self._card(summary, 2, "TOTAL técnico do pedido", self.order_cost_value)
        self._card(summary, 3, "Diferença vs Excel (TOTAL)", self.diff_value)
        ...
        heads = {
            ...
            "measure":"Medida","qty":"Qtd. na esquadria","unit_price":"Preço-base (R$/m, m² ou un.)","cost":"Custo na esquadria",
            "order_cost":"Custo total no pedido"
        }
        widths = {
    ...
    def _render(self,cfg,result):
        ...
        if raw:
            try:
                expected=parse_number(raw,"TOTAL esperado no Excel")
                engine_total, diff, pct = compare_order_total(result.unit_cost, cfg.quantity, expected)
    ...
    def save_test_case(self):
        ...
        engine_order_total = self.current_result.unit_cost * self.current_cfg.quantity
        raw=self.v["excel_cost"].get().strip()
        if raw:
            expected=parse_number(raw,"TOTAL esperado no Excel")
            engine_order_total, diff, diff_pct = compare_order_total(
                self.current_result.unit_cost, self.current_cfg.quantity, expected
            )
        ...
        row={
            ...
            "engine_order_total":engine_order_total,
            "excel_expected_order_total":"" if expected is None else expected,
            "difference_total_value":"" if diff is None else diff,
            "difference_total_pct":"" if diff_pct is None else diff_pct,
        ...
        csv_path=self._cases_dir()/"golden_cases_v0_2_1.csv"
        ...
        json_path.write_text(json.dumps({
            ...
            "expected_excel":{"order_total":expected},
            "engine_result":{
                "calculation_version":res.calculation_version,
            ...
            "comparison":{"engine_order_total":engine_order_total,"difference_total_value":diff,"difference_total_pct":diff_pct},
            "notes":self.v["notes"].get().strip(),
    ...
    def export_bom_csv(self):
        if not self.current_cfg or not self.current_result:
            messagebox.showwarning("Sem cálculo","Calcule antes de exportar o BOM.",parent=self); return
        target=filedialog.asksaveasfilename(
            parent=self,title="Exportar BOM para comparar com Excel",defaultextension=".csv",
            filetypes=[("Arquivo CSV","*.csv")],initialfile="bom_comparacao_excel.csv"
        )
        if not target: return
        cfg=self.current_cfg
        headers=[
            "grupo","componente","codigo","descricao","unidade","comprimento_mm",
            "largura_mm","altura_mm","area_m2","quantidade_na_esquadria",
            "quantidade_no_pedido","preco_base","custo_na_esquadria","custo_total_pedido"
        ]
        with Path(target).open("w",newline="",encoding="utf-8-sig") as f:
            writer=csv.writer(f,delimiter=";")
            writer.writerow(headers)
            for c in self.current_result.unit_bom:
                writer.writerow([
                    c.category, ROLE_LABELS.get(c.role,c.role), c.material_code, c.description, c.unit,
                    c.length_mm, c.width_mm, c.height_mm, c.area_m2, c.quantity_per_unit,
                    c.quantity_order, c.unit_price, c.cost_per_unit_product,
                    c.cost_per_unit_product*cfg.quantity
                ])
        self.status_var.set(f"BOM exportado: {Path(target).name}")
