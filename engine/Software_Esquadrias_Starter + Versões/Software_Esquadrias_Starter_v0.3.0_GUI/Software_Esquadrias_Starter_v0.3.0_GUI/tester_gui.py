...
class EsquadriasTester(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Software Esquadrias — Tester CR v0.3.0")
    ...
    def _build(self):
        ...
        ttk.Label(
            header, text="Software Esquadrias — Tester CR v0.3",
            style="Title.TLabel"
        ).pack(anchor="w")
        ttk.Label(
            header,
    ...
    def _results(self, parent):
        ...
        self._scrollable(bom_tab, self.bom_tree)

        ocols = ("idx", "application", "system", "size", "qty", "cost")
        self.order_tree = ttk.Treeview(order_tab, columns=ocols, show="headings")
        olabs = {
            "idx":"Item","application":"Aplicação","system":"Tipo de folha",
            "size":"L × A","qty":"Qtd.","cost":"Custo técnico"
        }
        owidth = {"idx":55,"application":90,"system":220,"size":150,"qty":70,"cost":130}
        for c in ocols:
        ...
        self._scrollable(purchase_tab, self.purchase_tree)
        ...
        self._scrollable(cut_tab, self.cut_tree)
    ...
    def _scrollable(self, parent, tree):
        frame = ttk.Frame(parent)
        frame.pack(fill="both", expand=True)
        # reparent workaround not possible; grid tree directly in existing parent
        tree.pack_forget()
        tree.master = parent
    ...
    def _render_order(self):
        ...
        for idx, (cfg, result) in enumerate(self.order_items, start=1):
            self.order_tree.insert("", "end", values=(
                ...
                f"{fmt_mm(cfg.width_mm)} × {fmt_mm(cfg.height_mm)}",
    ...
    def _render_warnings(self, plan=None):
        ...
        if warnings:
            ...
            self.warning_text.insert("end", "Nenhum alerta.")
